...
fn test_cypher_two_vars() {
    ...
    let mut property_cache = PropertyCache::new(Some(Arc::new(property_graph)), 6, 6, false, false);
    ...
    let result: Vec<u32> = vec
        .into_iter()
        .filter(|x| filter_node(*x, &mut property_cache, exp["0"].as_ref()))
        .collect();
...
fn test_cypher_two_vars2() {
    ...
    let result: Vec<u32> = vec
        .into_iter()
        .filter(|x| filter_node(*x, &mut property_cache, exp["a"].as_ref()))
        .collect();
...
fn test_cypher_two_vars4() {
    ...
    let exp = parse_property_tree(result);
    ...
    assert_eq!(exp.is_disabled(), true);
}
#[test]
