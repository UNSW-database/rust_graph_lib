...
fn test_cvs_unlabeled() {
    ...
    assert!(write_to_csv(&g, path_to_nodes.clone(), path_to_edges.clone()).is_ok());

    let mut g_ = GraphMap::new();
    assert!(read_from_csv(&mut g_, path_to_nodes, path_to_edges).is_ok());
    assert_eq!(g, g_);
...
fn test_cvs_labeled() {
    ...
    let mut g_ = GraphMap::with_label_map(
        SetMap::from_vec(node_labels.clone()),
        SetMap::from_vec(edge_labels.clone()),
    );
    assert!(read_from_csv(&mut g_, path_to_nodes, path_to_edges).is_ok());
    assert_eq!(g, g_);
    ...
    let mut g_ =
        GraphMap::with_label_map(SetMap::from_vec(node_labels), SetMap::from_vec(edge_labels));
    assert!(read_from_csv(&mut g_, path_to_nodes, path_to_edges).is_ok());
    assert_eq!(g, g_);
}
