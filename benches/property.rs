...
fn bench_cached(c: &mut Criterion) {
    let mut benchmark = Benchmark::new("cached_property_graph", |b| b.iter(|| cached_property_expression()));
    let result = benchmark.sample_size(10);
    c.bench(
        "my_group",
        result,
    );

//    c.sample_size(10).bench_function("Cached Property Graph", |b| b.iter(|| cached_property_expression()));
}

fn bench_sled(c: &mut Criterion) {
    let mut benchmark = Benchmark::new("sled_property_graph", |b| b.iter(|| sled_property_expression()));
    let result = benchmark.sample_size(5);
    c.bench(
        "my_group",
        result,
    );
//    c.sample_size(10).bench_function("Sled Property Graph", |b| b.iter(|| sled_property_expression()));
}

criterion_group!(benches, bench_cached, bench_sled);
criterion_main!(benches);


fn cached_property_expression() {
    ...
    let nodes = vec![0; 100000];

    property_filter.pre_fetch(&nodes, &property_graph);
...
//
//    db
    SledProperty::new(path, false).unwrap()
}
...
#[test]
fn test_sled_property_expression() {
    // WHERE a.is_member AND ((a.age MODULE 5 = 0) AND (18 <= a.age <= 35 AND ((a.name CONTAINS "a") OR (a.name CONTAINS "o"))));
