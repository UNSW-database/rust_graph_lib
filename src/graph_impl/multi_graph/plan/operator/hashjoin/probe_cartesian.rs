...
impl<Id: IdType> CommonOperatorTrait<Id> for ProbeCartesian<Id> {
    fn init<NL: Hash + Eq, EL: Hash + Eq, Ty: GraphType, L: IdType>(
        ...
        if self.base_probe.base_op.probe_tuple.len() == 0 {
    ...
    fn process_new_tuple(&mut self) {
        for a_hash_vertex in 0..self.highest_vertex_id {
            ...
            base_probe.base_op.probe_tuple[base_probe.hashed_tuple_len] = Id::new(a_hash_vertex);
    ...
    fn is_same_as(&mut self, op: &mut Operator<Id>) -> bool {
        if let Operator::Probe(Probe::PC(pc)) = op {
