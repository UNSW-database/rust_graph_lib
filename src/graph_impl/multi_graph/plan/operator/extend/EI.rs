...
use graph_impl::multi_graph::plan::operator::operator::{BaseOperator, CommonOperatorTrait, Operator};
...
impl<Id: IdType> BaseEI<Id> {
    ...
    fn set_operator_name(&mut self) {
        let variables = self.alds.iter()
            .map(|ald| {
                ald.from_query_vertex.clone() + "[" + if let Direction::Fwd = ald.direction { "Fwd" } else { "Bwd" } + "]"
            }).sorted().join("-");
        if 1 == self.alds.len() {
        ...
        self.base_op.name += &(" TO (".to_owned() + &self.to_query_vertex + ") From (" + &variables + ")");
    }

    pub fn make(to_qvertex: String, to_type: usize, alds: Vec<AdjListDescriptor>,
                out_subgraph: QueryGraph, in_subgraph: QueryGraph,
                out_qvertex_to_idx_map: HashMap<String, usize>) -> EI<Id> {
        if 1 == alds.len() {
            return EI::Extend(Extend::new(to_qvertex, to_type, alds, Box::new(out_subgraph),
                                          Some(Box::new(in_subgraph)), out_qvertex_to_idx_map));
        }
        EI::Intersect(Intersect::new(to_qvertex, to_type, alds, Box::new(out_subgraph),
                                     Some(Box::new(in_subgraph)), out_qvertex_to_idx_map))
    }
    ...
    pub fn is_intersection_cached(&mut self) -> bool {
        ...
        for i in 0..self.last_vertex_ids_intersected.as_ref().unwrap().len() {
            if self.last_vertex_ids_intersected.as_ref().unwrap()[i] != self.base_op.probe_tuple[self.vertex_idx_to_cache[i]] {
                self.is_intersection_cached = false;
                self.last_vertex_ids_intersected.as_mut().unwrap()[i] = self.base_op.probe_tuple[self.vertex_idx_to_cache[i]];
            }
        }
    ...
    pub fn init_caching(&mut self, last_repeated_vertex_idx: usize) {
        ...
        let num_cached_alds = self.alds.iter().filter(|&ald| ald.vertex_idx <= last_repeated_vertex_idx).count();
    ...
    fn init_extensions<NL: Hash + Eq, EL: Hash + Eq, Ty: GraphType, L: IdType>(&mut self, graph: &TypedStaticGraph<Id, NL, EL, Ty, L>) {
        ...
        for ald in &self.alds {
            let label = if graph.is_sorted_by_node() { self.to_type } else { ald.label };
    ...
    fn set_alds_and_adj_lists<NL: Hash + Eq, EL: Hash + Eq, Ty: GraphType, L: IdType>(&mut self, graph: &TypedStaticGraph<Id, NL, EL, Ty, L>, last_repeated_vertex_idx: usize) {
    ...
    pub fn execute_intersect(&mut self, idx: usize, intersect_type: IntersectType) -> usize {
        let (adj_vec, label_or_type) = match intersect_type {
            ...
            _ => (
                self.adj_lists_to_cache[idx][self.base_op.probe_tuple[self.vertex_idx_to_cache[idx]].id()].as_ref(),
